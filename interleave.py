...
def process_reads(args):
    ...
    while 1: 
        ...
        corrected_reading = 0 if args.rm_short_reads==None else args.rm_short_reads

        if len(left_seq.strip()) <= corrected_reading:
            continue

        if len(right_seq.strip()) <= corrected_reading:
            continue
